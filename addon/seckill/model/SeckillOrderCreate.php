...
class SeckillOrderCreate extends OrderCreate
{
	...
	public function calculate($data)
	{
		...
		$seckill_info_result = $seckill_model->getSeckillInfo([ [ "seckill_id", "=", $seckill_id ] ]);
...
//            return $cache;
//        }
		$goods_list = $this->getSeckillGoodsInfo($data["id"], $data['num']);
...
//        Cache::tag("order_create_seckill_".$data['member_id'])->set("order_create_seckill_".$data['id'].'_'.$data['num'].'_'.$data['member_id'], $shop_goods_list, 600);
	 ...
	 * @param unknown $bl_id
	 */
	public function getSeckillGoodsInfo($id, $num)
	{
		//组装商品列表
