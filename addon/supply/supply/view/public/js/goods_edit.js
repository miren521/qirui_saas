...
function refreshSkuTable() {
	...
	for (var j = 0; j < goodsSkuData.length; j++) {
		...
		var price_array =  item_goods_sku_data.price_json != ''? JSON.parse(item_goods_sku_data.price_json) : [{num:'',price:''}];
	...
	laytpl(sku_template).render(data, function (html) {
		...
		//绑定SKU列表中输入框键盘事件
		$(".sku-table .layui-input-block").on('keyup', 'input', function () {
...
refreshGoodsSkuData = function () {
	...
	if ($("input[name='goods_id']").length == 1) {
		$(".js-edit-sku-list>div").each(function (i) {
			...
			if (!goodsSkuData[i].price_json) goodsSkuData[i].price_json = $(this).children("input[name='edit_price_json']").val();
