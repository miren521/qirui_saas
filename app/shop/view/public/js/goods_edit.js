			...
			for (var x = length - 1; x >= 0; x--) {
				...
				for (var i = 0; i < goodsSkuData.length;) {
					if (goodsSpecFormat[x]['value'].length > 0) {
						/* for (var j = 0; j < goodsSpecFormat[x]['value'].length; j++) {
							$('.sku-table .layui-input-block table tbody tr:eq(' + i + ')').prepend('<td rowspan="' + c_n + '">' + goodsSpecFormat[x]['value'][j].spec_value_name + '</td>');
							i = i + c_n;
						} */
						...
						goodsSpecFormat[x]['value'].forEach(function (item, index) {
							$('.sku-table .layui-input-block table tbody tr:eq(' + i + ')').prepend('<td rowspan="' + c_n + '">' + item.spec_value_name + '</td>');
							i = i + c_n;
						})
					} else {
						i++;
...
refreshGoodsSkuData = function () {
	...
	arr.forEach(function (ele_1, i) {
		var item_prop_arr = [];
		if (goodsSkuData.length > 0) {
			
			goodsSkuData.forEach(function (ele_2, j) {
				ele_1['value'].forEach(function (ele_3, k) {
			...
			ele_1['value'].forEach(function (ele_3, l) {
...
function MonitorScrollBar(){
	var width = $(".ns-second-nav").width() + $(".ns-body-content").width(),
		htmlWidth = $("html").width();

	if(htmlWidth < width){
		$(".fixed-btn").css("bottom",16)
	}else{
		$(".fixed-btn").css("bottom",0)
	}
