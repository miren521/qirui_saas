...
class Diy extends BaseAdmin
{
    ...
    public function index()
    {
        ...
        // 查询公共组件和支持的页面
        $condition = [
            ['support_diy_view', 'like', [$page['admin']['index']['name'], '%' . $page['admin']['index']['name'] . ',%',
                '%' . $page['admin']['index']['name'], '%,' . $page['admin']['index']['name'] . ',%', ''], 'or']
        ];
        $data      = [
            'app_module' => $this->app_module,
            'site_id'    => $this->site_id,
            'name'       => $page['admin']['index']['name'],
            'condition'  => $condition
        ];
    ...
    public function goodsCategory()
    {
        ...
        $data      = [
            'app_module' => $this->app_module,
            'site_id'    => $this->site_id,
            'name'       => $page['admin']['goods_category']['name'],
            'condition'  => $condition
        ];
        $edit_view = event('DiyViewEdit', $data, true);
        return $edit_view;
    ...
    public function bottomNavDesign()
    {
        ...
        if (request()->isAjax()) {
            $value = input("value", "");
            $res   = $diy_view->setBottomNavConfig($value);
            return $res;
        } else {
            $bottom_nav_info = $diy_view->getBottomNavConfig();
    ...
    public function style()
    {
        ...
        if (request()->isAjax()) {
            ...
            $res = $config_model->setConfig($data, '店铺风格设置', '1', [['site_id', '=', 0], ['app_module', '=', 'admin'], ['config_key', '=', 'SHOP_STYLE_CONFIG']]);
            return $res;
        }
