...
Vue.component("title-empty", {
	...
	methods: {
		...
		verify : function () {
			...
			if (this.$parent.data.title.length > 24) {
				res.code = false;
			...
			if (this.$parent.data.isOpenOperation) {
				if (this.$parent.data.operation_name.length == 0) {
					res.code = false;
				...
				if (this.$parent.data.operation_name.length > 10) {
					res.code = false;
