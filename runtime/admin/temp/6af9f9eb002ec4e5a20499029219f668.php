<?php /*a:2:{s:60:"/www/wwwroot/saas.goodsceo.com/app/admin/view/pc/deploy.html";i:1600312146;s:55:"/www/wwwroot/saas.goodsceo.com/app/admin/view/base.html";i:1600312146;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div class="layui-collapse ns-tips">
	<div class="layui-colla-item">
		...
		<ul class="layui-colla-content layui-show">
			...
			<li>（难度：较高）源码下载：下载开源代码包，参考开发文档进行配置，结合业务需求进行二次开发。</li>
...
<div class="layui-form ns-form">
	...
	<div class="layui-form-item">
		...
		<div class="layui-input-block">
			<input type="radio" name="deploy_way" value="cs_default" lay-filter="deploy_way" title="默认部署" data-desc="无需下载，一键刷新，API接口请求地址为当前域名，编译代码存放到web文件夹中" checked />
			<input type="radio" name="deploy_way" value="cs_indep" lay-filter="deploy_way" title="独立部署" data-desc="下载编译代码包，参考开发文档进行配置"/>
			<input type="radio" name="deploy_way" value="os" lay-filter="deploy_way" title="源码下载" data-desc="下载开源代码包，参考开发文档进行配置，结合业务需求进行二次开发" />
		</div>
		<div class="ns-word-aux js-desc"></div>
	</div>

	<div class="layui-form-item js-domain">
		<label class="layui-form-label"><span class="required">*</span>域名地址：</label>
		<div class="layui-input-block">
			<input type="text" name="domain" lay-verify="domain" value="<?php echo htmlentities($root_url); ?>" autocomplete="off" class="layui-input ns-len-long">
		</div>
		<div class="ns-word-aux">API接口请求地址默认为当前域名</div>
	</div>
	...
	layui.use(['form'], function() {
		...
		form.verify({
			domain : function(value, item) {
				...
				if($("input[name='deploy_way']:checked").val() == "cs_indep") {
					if (value === '') {
						return "请输入域名地址";
		...
		form.on("submit(save)",function(data){
			...
			switch (data.field.deploy_way) {
				case "cs_default":
					...
					repeat_flag = true;
					$.ajax({
						url: ns.url("admin/pc/downloadcsdefault"),
						dataType: 'JSON',
						type: 'POST',
						success: function(res){
							repeat_flag = false;
				...
				case "cs_indep":
					...
					window.open(ns.url("admin/pc/downloadcsindep",data.field));
					break;
				case "os":
					window.open(ns.url("admin/pc/downloados"));
					break;
			}
		})
